                ...
                <Grid Margin="10">
                    ...
                    <Border Grid.Row="3"
                        ...
                        <TextBlock TextWrapping="Wrap">
                            Host baseline: MSiSCSI Automatic/Running; Multipath-IO installed; PURE FlashArray registered with Microsoft DSM; Global/default MPIO policy = Round Robin; Pure MPIO timers; High Performance power plan. The global/default policy does not prove the effective policy of an already-presented Pure MPIO device. Target portals and iSCSI sessions are configured only from the dedicated iSCSI Connections tab. The tool does not create Pods, volumes, or LUN mappings.
                        </TextBlock>
                    </Border>
...
function Refresh-WindowsResultsTree {
    ...
    foreach ($Row in @($script:WindowsResults | Sort-Object Host)) {
        ...
        $LabelCol.Width = New-Object System.Windows.GridLength(205)
        ...
        $Pairs = @(
            ...
            @("Global/default MPIO policy",[string]$Row.LBPolicy),
...
function Test-IscsiConnectionPostVerification {
    ...
    foreach ($HostName in @($HostCache.Keys | Sort-Object)) {
        ...
        foreach ($Policy in $Policies) {
            ...
            $IsRR = (
                $PolicyText -eq "RR" -or
                $PolicyText -eq "ROUND ROBIN"
            )

            $IsRRWS = (
                $PolicyText -eq "RRWS" -or
                $PolicyText -match "ROUND\s*ROBIN\s*WITH\s*SUBSET"
            )
            ...
            $Line =
                "$HostName disk=$($Policy.DiskNumber) observedDevicePolicy=$($Policy.Policy) paths=$PathText expected=$($Expectation.Expected)"

            Write-ToolLog "MPIO DEVICE VERIFY $Line." "INFO"
            ...
            elseif ($IsRRWS) {
                Write-ToolLog (
                    "MPIO DEVICE VERIFY INFO $HostName disk=$($Policy.DiskNumber): observed device policy is RRWS (Round Robin with Subset). Observation only; the tool does not change per-device MPIO policy."
                ) "INFO"
            }
            elseif ($PathCount -gt 10 -and -not $IsLQD) {
                $Failures.Add(
                    "$HostName disk $($Policy.DiskNumber): $PathCount paths observed; LQD is expected for 11-32 Pure paths. Per-device policy was not changed."
                )
            }
            elseif ($PathCount -gt 0 -and -not ($IsRR -or $IsLQD)) {
                $Failures.Add(
                    "$HostName disk $($Policy.DiskNumber): observed device policy '$($Policy.Policy)' is not RR or LQD for $PathCount paths. Per-device policy was not changed."
                )
            }
        }
    ...
    $Message = (
        ...
        [Environment]::NewLine +
        [Environment]::NewLine +
        "No disks were initialized or formatted, and no per-device MPIO policy was changed."
    )
...
function Apply-IscsiConnections {
    ...
    $ConfirmText = (
        ...
        [Environment]::NewLine +
        "It will NOT create or map Pure volumes, initialize or format disks, change per-device MPIO policy, modify preferred-array settings, create CSVs, or modify Failover Cluster configuration."
    )
...
$AuditScript = {
    ...
    if ($Feature.Installed) {
        ...
        try {
            ...
            try {
                # Host/global default only. This does not prove an existing Pure device's effective policy.
    ...
    if ($Feature.Installed -and $Result.LBPolicy -notmatch "RR|Round") {
        $Issues.Add("Global/default MPIO policy")
    }
...
$ConfigureScript = {
    ...
    try {
        ...
        if ([string]$Policy -notmatch "RR|Round") {
            ...
            $Actions.Add("Global/default MPIO policy=RR")
        }
...
function Show-ToolHelp {
    ...
    $Message = @"
iSCSI HOST TOOL v2.5.1 - QUICK START
...
- Set Microsoft DSM global/default MPIO policy to Round Robin
- Apply Pure-recommended global MPIO timers`n`nMPIO policy note:`nThe tool configures the Microsoft DSM global/default load-balancing policy. Existing Pure devices can report a different effective per-device policy based on Windows MPIO/ALUA behavior. The tool does not silently change per-device MPIO policy.
...
Global/default MPIO policy: $($Selected.LBPolicy)

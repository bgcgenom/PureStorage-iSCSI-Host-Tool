...
function Update-SessionStateBanner {
    ...
    $SessionStateText.Text =
        "Audit: $AuditText | Arrays: $ConnectedArrays/$TotalArrays Connected | Validation: $ValidationText$VerifyText"
...
function Reset-ToolSession {
    $Confirm =
        [System.Windows.MessageBox]::Show(
            "Reset the current working session?`n`nThis clears:`n- Windows audit results/state`n- in-memory Windows credential`n- Pure validation results/plan`n- Apply readiness`n`nActive array connections remain connected until the application closes.`nSaved encrypted array definitions are retained.",
            "Reset Session",
            "YesNo",
            "Warning"
        )
...
function Show-ToolHelp {
    ...
    $Message = @"
iSCSI HOST TOOL v2.4.19 - QUICK START
...
11. Apply only when the plan is correct.
12. Verify the post-Apply result.
13. Use Export All to package audit, validation, preview, summary, and log.
14. Use Reset Session when finished or before starting a different workflow.
